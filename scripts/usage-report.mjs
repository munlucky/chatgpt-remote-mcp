...
const groups = new Map(); let malformed = 0; let samples = 0;
for (const file of (await readdir(dir)).filter(f => /^requests\.jsonl(?:\.\d+)?$/.test(f))) {
  ...
  for await (const line of reader) {
    ...
    if (e.event !== 'mcp_request' || e.trafficClass === 'probe' || !(Date.parse(e.timestamp) >= since)) continue;
    if (!Number.isFinite(e.durationMs)) { malformed++; continue; }
    const key = `${e.buildId}|${e.toolName || e.rpcMethod || 'http'}`;
    if (!groups.has(key)) groups.set(key, { buildId: e.buildId, call: e.toolName || e.rpcMethod || 'http', count: 0, errors: 0, aborted: 0, duration: [], setup: [], tool: [], bytes: [] });
...
console.log(JSON.stringify({ generatedAt: new Date().toISOString(), hours, samples, malformed, note: 'Retained rolling window only; marked probes excluded. Sparse samples and different commands are not comparable performance proof. Tool time includes deliberate execution waits.', groups: [...groups.values()].map(g => ({ buildId:g.buildId, call:g.call, count:g.count, errors:g.errors, aborted:g.aborted, p50Ms:percentile(g.duration,.5), p95Ms:percentile(g.duration,.95), setupP95Ms:percentile(g.setup,.95), toolP95Ms:percentile(g.tool,.95), responseBytesP95:percentile(g.bytes,.95) })).sort((a,b)=>b.count-a.count) }, null, 2));
