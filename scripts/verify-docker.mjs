...
execFileSync('docker',['run','--rm','--entrypoint','/bin/bash','-w','/opt/chatgpt-remote-mcp','chatgpt-remote-mcp:verify','-c','npm run typecheck && npm test && npm run build && npm audit --omit=dev && node scripts/benchmark-http.mjs'],{stdio:'inherit'});
...
if (process.argv.includes('--live') || process.env.MCP_VERIFY_LIVE === '1') execFileSync(process.execPath,['scripts/verify-live.mjs'],{stdio:'inherit'});
