...
export class FileService {
  ...
  async listDirectory(
    ...
    const visit = async (directory: string, depth: number): Promise<void> => {
      ...
      for (const entry of directoryEntries) {
        ...
        // Dirent already supplies type on normal filesystems. Only request
        // metadata (or recover an unknown type) with another filesystem call.
        const knownType = entry.isFile() || entry.isDirectory() || entry.isSymbolicLink()
          || entry.isBlockDevice() || entry.isCharacterDevice() || entry.isFIFO() || entry.isSocket();
        const info = includeMetadata || !knownType ? await lstat(absolutePath) : entry;
        ...
        if (includeMetadata && "size" in info) {
  ...
  async applyPatch(
    ...
    const temporaryDirectory = await mkdtemp(
      path.join(os.tmpdir(), "chatgpt-remote-mcp-patch-"),
    );
    ...
    if (!overwrite) {
      ...
      try {
        ...
        destinationBackup = path.join(
          ...
          `.chatgpt-remote-mcp-move-backup-${randomUUID()}`,
        );
