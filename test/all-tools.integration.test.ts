...
function externalRoot(): string | undefined {
  ...
  if (!/^\/tmp\/chatgpt-remote-mcp-tools-e2e-[A-Za-z0-9._-]+$/.test(value)) {
    throw new Error(
      "MCP_E2E_ROOT must be an isolated /tmp/chatgpt-remote-mcp-tools-e2e-* path",
    );
  }
  return value;
}
...
describe.sequential("all registered MCP tools", () => {
  ...
  beforeAll(async () => {
    ...
    if (externalUrl) {
      ...
      testRoot = externalRoot() ?? `/tmp/chatgpt-remote-mcp-tools-e2e-${randomUUID()}`;
      endpoint = new URL(externalUrl);
    } else {
      localDirectory = await mkdtemp(path.join(os.tmpdir(), "chatgpt-remote-mcp-all-tools-"));
  ...
  it("executes, polls, writes to, times out, lists, and terminates processes", async () => {
    ...
    expect(String(script.scriptPath)).toMatch(/^\/tmp\/chatgpt-remote-mcp-script-/);
  ...
  it("reads multiple files with independent per-file errors", async () => {
